...
DATA_PATH = Path(__file__).parent / "REPORTE_CONSOLIDADO_MOVILIDAD_HUMANA.xlsx"
DATA_CACHE_PATH = DATA_PATH.with_suffix(".cache.pkl")
DATA_CACHE_VERSION = 3


def _ensure_data_file() -> None:
    """Si el .xlsx no existe localmente (ej. en Streamlit Cloud), lo descarga
    desde Google Drive usando el FILE_ID guardado en st.secrets. Nunca se
    hardcodea el ID ni el archivo se sube a Git; el secreto se define en
    .streamlit/secrets.toml (local, gitignored) o en el panel de Streamlit
    ...
    if not file_id:
        return  # se maneja el error más abajo en load_data()
...
def load_data() -> pd.DataFrame:
    ...
    if not DATA_PATH.exists():
        st.error(
            f"No se encontró el archivo de datos: {DATA_PATH.name}\n\n"
            "Colócalo junto a app.py, o configura el secreto DRIVE_FILE_ID "
            "para descargarlo automáticamente desde Google Drive."
        )
        st.stop()
